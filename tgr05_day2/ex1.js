...
var mqtt_client = mqtt.connect("mqtt://" + config.mqtt.host, {username: config.mqtt.username, password: config.mqtt.password, clientId: 'tgrxx', connectTimeout: 3000, reconnectPeriod: 3000})
...
mqtt_client.on('message', function (topic, message) {
    if(topic.toString().match(/tgr2020\/.+/g)) {
        ...
        var sensor_id = topic.toString().split('/')[3] //ไม่รู็ใช้ตัวไหน
        var tmp = {}

        tmp['ts'] = new Date() /เวลา
